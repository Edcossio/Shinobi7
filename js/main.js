document.addEventListener("DOMContentLoaded", () => {
    ...
    // ==========================================================
    // LOGIN (Corregido con btoa y limpieza de sintaxis)
    // ==========================================================
    ...
    function actualizarInterfazSesion() {
        ...
        const sesionActiva = sesionGuardada ? JSON.parse(sesionGuardada) : null;
        ...
        // PANEL ADMIN
        if (sesionActiva.rol === "Administrador" && navLinks && !document.getElementById("link-panel-admin")) {
            const liAdmin = document.createElement("li");
            liAdmin.innerHTML = `
                <a href="admin_home.html" id="link-panel-admin" class="nav-item fw-bold text-danger">ADMIN</a>
            `;
        ...
        enlacesNav.forEach(enlace => {
            if (enlace.textContent.includes("INICIAR SESIÓN")) {
                const primerNombre = sesionActiva.nombre ? sesionActiva.nombre.split(" ")[0] : "";
                enlace.textContent = `CERRAR SESIÓN (${primerNombre})`;
                enlace.href = "#";
    ...
    const sesionActiva = actualizarInterfazSesion();

    // ==========================================================
    // DATOS DE DESPACHO
    // ==========================================================
    function actualizarDespacho() {
        if (!document.getElementById("contenedor-carrito-items")) return;
        const direccionInput = document.getElementById("direccion-despacho");
        if (!direccionInput) return;

        const sesionGuardada = sessionStorage.getItem("sesionActiva");
        const usuario = sesionGuardada ? JSON.parse(sesionGuardada) : null;

        if (!usuario) {
            direccionInput.value = "Inicia sesión para ver tu dirección";
        ...
        if (usuario.direccion) {
            ...
            direccionInput.readOnly = true;
            direccionInput.style.cursor = "default";
            const direccionTexto = document.getElementById("direccion-usuario");
            if (direccionTexto) direccionTexto.textContent = usuario.direccion;
        }
    }
    actualizarDespacho();
    ...
    if (formRegistro) {
        formRegistro.addEventListener("submit", e => {
            ...
            const telefono = telefonoElemento ? telefonoElemento.value.trim() : "";
            const direccionElemento = document.getElementById("direccion");
            const direccion = direccionElemento ? direccionElemento.value.trim() : "";
            ...
            const checkRun = validarRun(run);
            if (!checkRun.valido) return mostrarMensaje(checkRun.msj);
            ...
            if (!region) return mostrarMensaje("Debes seleccionar una región.");
            if (!comuna) return mostrarMensaje("Debes seleccionar una comuna.");

            if (!direccion || direccion.length > 150) {
                return mostrarMensaje("Debes ingresar una dirección válida (máx. 150 caracteres).");
            }
            ...
            const nuevoUsuario = {
                ...
                nombre: nombre,
                correo: correo,
                password: btoa(pass), // <-- Contraseña codificada de forma segura
                telefono: telefono,
                region: region,
                comuna: comuna,
                direccion: direccion,
